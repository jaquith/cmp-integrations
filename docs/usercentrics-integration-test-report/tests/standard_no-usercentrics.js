...
describe('STEP 1 - initial visit to test page', function () {
  it('should navigate to the page', async function () {
    await browser.url(helper.standardNoUsercentricsTestPage);
  ...
  it('should NOT have a utag_main cookie', async function () {
    await browser.waitForTraffic();
    reporterHelper.logMessage('TiQ isn\'t even allowed to load');
    const cookie = await browser.getCookieByName('utag_main');
    chai.expect(cookie).to.be.undefined();
  });

  it('should NOT have a visible Usercentrics banner', async function () {
    const ucBanner = await $(helper.selectors.ucShadow);
    chai.expect(await ucBanner.isExisting()).to.be.false();
  });
...
describe('STEP 2 - (no Usercentrics, so no consent decision)', function () {
  it('(no tracking to deny)', async function () {
